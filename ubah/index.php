...
$edit = query("SELECT * FROM pasien WHERE id = $id")[0];
...
if( isset($_POST["submit"]) ) {
	...
	if( edit($_POST, $id) > 0 ) {
		echo "
			<script>
				alert('Edit Successful'); document.location.href = '../datapasien/index.php';
			</script>
		";
	} else {
		echo "
			<script>
				alert('Edit Failed'); document.location.href = '../datapasien/index.php';
			</script>
		";
	}
...
<section id="daftar" class="pt-36 pb-32">
    <div class="container">
        <div class="w-full px-4 lg:w-2/3 lg:mx-auto">
            <h1 class="block font-bold text-dark text-3xl lg:text-5xl">Data Online <span class="text-base font-semibold text-primary md:text-xl lg:text-2xl">Pasien!</span></h1>
        </div>
        <div class="w-full lg:w-2/3 lg:mx-auto">
        <div class="w-full px-4 mb-8">
        <form action="" name="check" method="post" enctype="multipart/form-data" onclick="validateForm()" onsubmit="return validate()">


        <input type="hidden" name="id" value="<?= $edit["id"] ?>">
<div class="my-3">
    <label for="nama" class="text-base font-bold text-primary">Nama</label>
    <input class="w-full bg-slate-200 text-dark p-3 rounded-md focus:outline-none focus:ring-primary focus:ring-1 focus:border-primary" autofocus type="text" class="form-control" id="nama" name="nama" required minlength="2" autofocus autocomplete="off" value="<?= $edit["nama"] ?>">
</div>
...
<div class="mb-3">
    ...
    <input type="text" class="w-full bg-slate-200 text-dark p-3 rounded-md focus:outline-none focus:ring-primary focus:ring-1 focus:border-primary" id="alamat" name="alamat" value="<?= $edit["alamat"] ?>" required autocomplete="off">
    
</div>

<div class="mb-3">
    <label for="email" class="text-base font-bold text-primary">Email</label>
    <input type="email" class="w-full bg-slate-200 text-dark p-3 rounded-md focus:outline-none focus:ring-primary focus:ring-1 focus:border-primary" id="email" name="email" value="<?= $edit["email"] ?>" required autocomplete="off">
...
<div class="mb-3">
    ...
    <input type="tel" class="w-full bg-slate-200 text-dark p-3 rounded-md focus:outline-none focus:ring-primary focus:ring-1 focus:border-primary" id="hp" name="hp" value="<?= $edit["hp"] ?>" required autocomplete="off" onkeypress="return hanyaAngka(event)" pattern="(\+62|62|0)8[1-9][0-9]{6,13}$">
    
</div>

<div class="mb-3">
    <label for="nik" class="text-base font-bold text-primary">No. KTP</label>
    <input type="number" pattern="{14,16}$" class="w-full bg-slate-200 text-dark p-3 rounded-md focus:outline-none focus:ring-primary focus:ring-1 focus:border-primary" id="nik" name="nik" value="<?= $edit["nik"] ?>" required autocomplete="off" onkeypress="return hanyaAngka(event)">
</div>
...
<select class="form-select w-full bg-slate-200 text-dark p-3 rounded-md focus:outline-none focus:ring-primary focus:ring-1 focus:border-primary" id="pl" name="pl">
    <option selected required="required" value="<?= $edit["pl"] ?>"></option>
...
<button class="text-base font-semibold text-white bg-primary py-3 px-8 rounded-full w-full hover:opacity-80 hover:shadow-lg transition duration-500" type="submit" name="submit" onsubmit="return validate()" class="btn btn-primary mt-3">Submit</button>
</div>
        </form>
        </div>
        </div>
